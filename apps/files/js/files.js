...
$(document).ready(function() {
  ...
  $('#fileList').on('click change', 'td.filename :checkbox', function(event) {
    if (!lastChecked && event.type == 'click') {
      lastChecked = this;
      return;
    }
    var $chkboxes = $('#fileList td.filename :checkbox');
    if (event.shiftKey) {
      var start = $chkboxes.index(this),
        end = $chkboxes.index(lastChecked);
      $chkboxes.slice(Math.min(start,end), Math.max(start,end)+ 1).attr('checked', lastChecked.checked);
    }
    var $selected = $('#fileList td.filename :checked'),
    ...
    if (!$(this).attr('checked')) {
      ...
      if (selectedCount === $chkboxes.length) {
        $('#select_all').attr('checked', true);
      }
    }
    if (event.type == 'click') { lastChecked = this; }
    procesSelection();
  });
  ...
  // Finally, update the selection properties (if the user has selected things while the script loaded)
  procesSelection();
});
...
function procesSelection() {
  var selected = getSelectedFiles(),
    selectedFiles = selected.filter(function(f){ return f.type=='file'; }),
    selectedFolders = selected.filter(function(f){ return f.type=='dir'; });
  if (selectedFiles.length == 0 && selectedFolders.length == 0) {
    ...
    if (selected.filter(function(f){ return !(f.permissions & OC.PERMISSION_DELETE); }).length > 0) {
      $('.selectedActions .delete-selected').hide();
    } else {
      $('.selectedActions .delete-selected').show();
    }
    var totalSize = 0;
    ...
    simpleSize = humanFileSize(totalSize);
 ...
 */
function getSelectedFiles(property) { return getFiles(true, null, property); }

/**
 * @brief get a list of files
 * @param boolean selected (option) if we want to restrict to selected files
 * @param string mime (option) the mime to filter by, if we want to restrict to files of a particular type
 * @param string property (option) the property of the file requested
 * @return array
 *
 * possible values for property: name, mime, size and type
 ...
 * if it's ommited an array of objects with all properties is returned
 */
function getFiles(selected, mime, property) {
  var elements = $('td.filename input:checkbox' + (selected ? ':checked' : '')).parent().parent(),
    files = [];
  elements.each(function(i,element){
    var file = {
      ...
      permissions: parseInt($(element).data('permissions'), 10)
    };
    if (mime && file.mime.split(/\//)[0] !== mime) {
      return;
    }
    if (property) {

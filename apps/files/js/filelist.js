var FileList={
  ...
  createRow:function(type, name, iconurl, linktarget, size, lastModified, permissions){
    ...
    if (!(permissions & OC.PERMISSION_UPDATE)) { tr.addClass('locked'); }
    // filename td
    td = $('<td></td>').attr({
      "class": "filename",
    ...
    link_elem.append('<span class="lock-icon" />');
  ...
  addFile:function(name,size,lastModified,loading,hidden){
    var imgurl,
      inShared = $('#dir').val().replace(/^\/+/, '').split('/')[0] == 'Shared',
      allPerms = OC.PERMISSION_DELETE | OC.PERMISSION_UPDATE | OC.PERMISSION_READ | OC.PERMISSION_SHARE;
    console.log($('#dir').val().replace(/^\/+/, '').split('/')[0]);
    if (loading) {
    ...
    var tr = this.createRow(
      ...
      size,
      lastModified,
      inShared ? $('#permissions').val() : allPerms
    );
  ...
  addDir:function(name,size,lastModified,hidden){
    var inShared = $('#dir').val().replace(/^\/+/, '').split('/')[0] == 'Shared',
      allPerms = OC.PERMISSION_CREATE | OC.PERMISSION_DELETE | OC.PERMISSION_UPDATE | OC.PERMISSION_READ 
        | OC.PERMISSION_SHARE;
    var tr = this.createRow(
      'dir',
      name,
  ...
  do_delete: function(files) {
    var deleteAction, oldHTML, newHTML;
    if(files.substr){
      files=[files];
    }
    for (var i=0; i<files.length; i++) {
      deleteAction = $('tr').filterAttr('data-file',files[i]).children("td.date").children(".action.delete");
      if (deleteAction[0]) {
        oldHTML = deleteAction[0].outerHTML;
        newHTML = '<img class="move2trash" data-action="Delete" title="'+t('files', 'perform delete operation')+'" src="'+ OC.imagePath('core', 'loading.gif') +'"></a>';
